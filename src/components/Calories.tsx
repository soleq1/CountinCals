...
const [calorieData, setCalorieData] = useState({
  ...
  const sendCal = () =>{
    ...
    if (prevCalorieData.current !== calorieData){
        try{
            const response = axios.post('http://localhost:3000/userCalorie',{calorieData ,uid})
   ...
   useEffect(() => {
    setCalorieData(prevData => ({
        ...prevData,
        calories: prevData.fat + prevData.carbs + prevData.protein
      }));
    if (calorieData.fat < 0) {
      setCalorieData(prevData => ({
        ...prevData,
        fat: 0
      }));
  ...
  }, [calorieData.fat, calorieData.carbs, calorieData.protein]);
  useEffect(() =>{
    console.log(uid)
  },[uid])
    ...
    return(
        <div className="Calorie-Container">
            ...
            {/* <div className="Calorie-BreakDown"> */}
                <div className="Calorie-BreakDown">Fat {calorieData.fat} <Arrows onPressUp={() => handleAdd('fat',9)} onPressDown={() => handleSub('fat',9)}  /> </div>
